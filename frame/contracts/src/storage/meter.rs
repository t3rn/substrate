...
impl<T: Config> Default for Cost<T> {
	fn default() -> Self {
		Self::Charge(<BalanceOf<T>>::zero())
	}
}
	...
	S: State,
{
	...
	pub fn absorb(
		&mut self,
		absorbed: &mut RawMeter<T, E, Nested>,
		origin: &T::AccountId,
		contract: &T::AccountId,
	) {
		E::charge(origin, &contract, &absorbed.own_usage.to_cost());
	...
	pub fn total_cost(&self) -> Cost<T> {
		self.total_usage.to_cost()
	}
	...
	E: Ext<T>,
{
	pub fn charge(&mut self, diff: &Diff) -> DispatchResult {
		...
		if let Cost::Charge(amount) = self.total_usage.to_cost() {
...
impl<T: Config> Ext<T> for DefaultExt {
	...
	fn charge(origin: &T::AccountId, contract: &T::AccountId, amount: &Cost<T>) {
		let (slashed, beneficiary, amount) = match amount {
			Cost::Charge(amount) => (origin, contract, amount),
			Cost::Refund(amount) => (contract, origin, amount),
		};
		...
		match result {
			Err(err) => log::error!(
				...
				"{:?}: beneficiary {} does not exist for storage deposit transfer. This is a bug.",
				err, beneficiary,
			),
			Ok(missing) => log::error!(
				target: "runtime::contracts",
				"Missing {:?} reserved balance from account {} for storage deposit transfer. This is a bug.",
				missing, slashed,
			),
		}
	}
