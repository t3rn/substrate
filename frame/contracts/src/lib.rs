...
use crate::{
	...
	gas::GasMeter,
	storage::{
		meter::{Cost as StorageCost, Meter as StorageMeter},
...
struct InternalOutput<T: Config, O> {
	...
	/// The costs generated by the call.
	storage_cost: StorageCost<T>,
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	fn internal_call(
		...
		InternalCallOutput { result, gas_meter, storage_cost: storage_meter.total_cost() }
	}
	...
	fn internal_instantiate(
		...
		InternalInstantiateOutput {
			result: try_exec(),
			gas_meter,
			storage_cost: storage_meter.total_cost(),
		}
	}
}
